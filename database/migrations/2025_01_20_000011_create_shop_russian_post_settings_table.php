...
return new class extends Migration
{
    ...
    {
        if (Schema::hasTable('shop_russian_post_settings')) {
            return;
        }

        Schema::create('shop_russian_post_settings', function (Blueprint $table) {
            $table->id();
            // Учетные данные Почты России
            $table->string('api_token')->comment('API ключ (Token) Почты России');
            $table->string('login')->comment('Логин для входа в личный кабинет');
            $table->string('password')->nullable()->comment('Пароль для входа в личный кабинет');

...
class OzonProductPayloadBuilderTest extends TestCase
{
    ...
    public function test_dimension_sources_prefer_variation_and_fall_back_to_parent_good(): void
    {
        ...
        $mapping = new ShopOzonCategoryMapping([
            ...
            'type_id' => 2,
            'attribute_mappings' => [
                ['id' => 101, 'source' => 'dimension_weight'],
                ['id' => 102, 'source' => 'dimension_width'],
        ...
        $this->assertSame('3.250', data_get($attributes->firstWhere('id', 101), 'values.0.value'));
        $this->assertSame('36.00', data_get($attributes->firstWhere('id', 102), 'values.0.value'));
        $this->assertSame('50.00', data_get($attributes->firstWhere('id', 103), 'values.0.value'));
        $this->assertSame('25.00', data_get($attributes->firstWhere('id', 104), 'values.0.value'));
        $this->assertSame('55.00', data_get($attributes->firstWhere('id', 105), 'values.0.value'));
    }
}

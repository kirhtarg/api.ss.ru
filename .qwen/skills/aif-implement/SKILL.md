...
Options:
1. Yes, commit now (/skills aif-commit)
...
- Yes → run `/skills aif-commit`, then continue to plan discovery
...
A fix plan was created by `/skills aif-fix` in plan mode. Redirect to fix workflow:

```
Found a fix plan at the resolved fix plan path.

This plan was created by /skills aif-fix and should be executed through the fix workflow
(it creates a patch and handles cleanup automatically).

Running /skills aif-fix to execute the plan...
```

→ **Invoke `/skills aif-fix`** (without arguments – it will detect the resolved fix plan and execute it).
...
- New feature from current → `/skills aif-plan full <description>`
- Return to base branch → `git checkout <configured-base-branch>`, then `git pull origin <configured-base-branch>` → `/skills aif-plan full <description>` (git mode only)
- Quick task → `/skills aif-plan fast <description>`
...
- Route it to `/skills aif-plan full <description>` without any git commands
...
3. If file does not exist:
   "Plan file not found: <path>
    ...
    - /skills aif-implement @<resolved fast plan path>
    - /skills aif-implement @.ai-factory/plans/feature-user-auth.md"
   → STOP
4. If file is the resolved fix plan path:
   → invoke /skills aif-fix (ownership + cleanup workflow) and STOP
...
2. If git mode is off or branch-based plan is missing:
   - Check whether the configured plans dir contains exactly one `*.md` plan file created by `/skills aif-plan full` without a branch
...
4. No full-mode plan and no resolved fast plan → Check the resolved fix plan path
   → If exists: invoke /skills aif-fix (handles its own workflow with patches) and STOP
```

**Priority:**

1. `@<path>` argument - explicit user-selected plan file
2. Branch-named file (from `/skills aif-plan full`) - if it matches current branch
3. Single named full-plan file in `paths.plans` (from `/skills aif-plan full` without branch creation)
4. `paths.plan` (from `/skills aif-plan fast`) - fallback when no full plan exists
5. `paths.fix_plan` - redirect to `/skills aif-fix` (from `/skills aif-fix` plan mode)
...
- Yes, commit now → invoke `/skills aif-commit` with the suggested message, then continue to next task
- No, continue to next task → proceed to the next task without committing
- Skip all commit checkpoints → for all subsequent checkpoints within this `/skills aif-implement` run, skip the prompt automatically and proceed directly to the next task (as if user selected "No, continue to next task" each time). This is in-context memory — resets on `/clear` or new session
...
1. 🔍 /skills aif-verify — Verify nothing was missed (recommended)
2. 💾 /skills aif-commit — Commit the changes directly
```
...
- If milestone mapping is ambiguous, emit `WARN [roadmap] ...` and suggest the owner command:
    - `/skills aif-roadmap check`
    - or `/skills aif-roadmap <short update request>`

**RULES.md (NOT allowed in this command):**

- Never edit the resolved `paths.rules_file` artifact from `/skills aif-implement`.
- If you discovered repeatable conventions/pitfalls during implementation, propose up to 3 candidate rules and ask the user to add them via `/skills aif-rules`.
- Do not invoke `/skills aif-rules` automatically (it is user-invoked).
...
Options:
1. Yes — output `/skills aif-rules ...` commands (recommended)
2. No — skip
```
...
Options:
1. Update existing docs (recommended) — invoke /skills aif-docs
2. Create a new feature doc page — invoke /skills aif-docs with feature-page context
3. Skip documentation
```

Handling:

- Option 1 → invoke `/skills aif-docs` to update README/docs based on completed work
- Option 2 → invoke `/skills aif-docs` with context to create `docs/<feature-slug>.md`, include sections (Summary, Usage/user-facing behavior, Configuration, API/CLI changes, Examples, Troubleshooting, See Also), and add a README docs-table link
- Option 3 → do not invoke `/skills aif-docs`; emit `WARN [docs] Documentation skipped by user`

If plan setting is `Docs: no` or setting is unset:

- Do **not** show a mandatory docs checkpoint prompt
- Do **not** invoke `/skills aif-docs` automatically
...
- **If the resolved fast plan path** (from `/skills aif-plan fast`):
...
- "No, I'll handle it manually" → show a reminder:
  ...
  To merge and clean up later:
    cd <main-repo-path>
    git merge <branch>
    /skills aif-plan --cleanup <branch>
  ```

#### Worktree Merge

1. **Ensure everything is committed** — check `git status`. If uncommitted changes exist, suggest `/skills aif-commit` first and wait.
...
Options:
1. Verify first — Run /skills aif-verify to check completeness (recommended)
2. Skip to commit — Go straight to /skills aif-commit
```

**Based on choice:**

- "Verify first" → invoke `/skills aif-verify` → after it completes, continue to context cleanup below
- "Skip to commit" → invoke `/skills aif-commit` → after it completes, continue to context cleanup below
...
```
/skills aif-implement @my-custom-plan.md
/skills aif-implement @.ai-factory/plans/feature-user-auth.md status
```
...
- ✅ Follow `/skills aif-best-practices` guidelines (naming, structure, error handling)

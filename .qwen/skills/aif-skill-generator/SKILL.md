...
```
If not found — ask user for path, offer to skip scan (at their risk), or suggest installing Python. If skipping, still perform Level 2 (manual review). See `/skills aif` skill for full detection flow.
...
0. Scope check (MANDATORY):
   ...
   - If path points to built-in AI Factory skills (.qwen/skills/skills aif or .qwen/skills/skills aif-*), this is wrong target selection for install-time security checks.
...
2. LEVEL 1 — Run automated scan:
   $PYTHON ~/.qwen/skills/skills aif-skill-generator/scripts/security-scan.py <skill-path>
...
- `/skills aif-skill-generator <name>` - Generate a new skill interactively
- `/skills aif-skill-generator <url> [url2] [url3]...` - **Learn Mode**: study URLs and generate a skill from them
- `/skills aif-skill-generator search <query>` - Search existing skills on skills.sh for inspiration
- `/skills aif-skill-generator scan <path>` - **Security scan**: run two-level security check on a skill
- `/skills aif-skill-generator validate <path>` - **Full validation**: structure check + two-level security scan
- `/skills aif-skill-generator template <type>` - Get a template (basic, task, reference, visual)
...
**Trigger:** `/skills aif-skill-generator scan <path>`
...
2. **LEVEL 1** — Run automated scanner:
   ```bash
   $PYTHON ~/.qwen/skills/skills aif-skill-generator/scripts/security-scan.py <path>
   ```
...
**Trigger:** `/skills aif-skill-generator validate <path>`
...
7. **AUTO-SCAN**: Run `/skills aif-skill-generator scan <generated-skill-path>` on the result
...
```bash
$PYTHON ~/.qwen/skills/skills aif-skill-generator/scripts/security-scan.py ./skill-name/
```

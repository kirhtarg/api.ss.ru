...
- Tell the user: "No fix plan found and no problem description provided. Please either provide a bug description (`/skills aif-fix <description>`) or create a fix plan first."
- **STOP.**
...
**User:** `/skills aif-fix TypeError: Cannot read property 'name' of undefined in UserProfile`
...
**User:** `/skills aif-fix /api/orders returns empty array for authenticated users`
...
**User:** `/skills aif-fix email validation accepts invalid emails`
...
11. **Ownership boundary** - `/skills aif-fix` owns `paths.fix_plan` and `paths.patches`; treat `.ai-factory/DESCRIPTION.md`, roadmap, rules, and architecture context artifacts as read-only unless the user explicitly requests otherwise

---
name: aif-improve
description: Refine and enhance an existing implementation plan with a second iteration. Re-analyzes the codebase, checks for gaps, missing tasks, wrong dependencies, and improves the plan quality. Use after /skills aif-plan to polish the plan before implementation, or to improve an existing /skills aif-fix plan.
...
4. Print availability summary and usage hints:
   - /skills aif-improve @<path> <optional prompt>
   - /skills aif-improve <optional prompt>      # automatic priority
5. If none found, suggest creating a plan via /skills aif-plan or /skills aif-fix
6. STOP.
```
...
2. No explicit `@<path>` override → Check current git branch:
   ...
   → Look for <configured plans dir>/<branch-name>.md (from /skills aif-plan full)
...
4. No full-mode plan → Check the resolved fast plan path (from /skills aif-plan fast)
5. No full-mode plan and no resolved fast plan → Check the resolved fix plan path (from /skills aif-fix plan mode)
```
...
- /skills aif-plan full <description>  — for a new feature (rich full plan; may also create a branch when git settings allow it)
- /skills aif-plan fast <description>  — for a quick task plan
- /skills aif-fix <bug description>    - for a bugfix plan (use the resolved fix plan path)
```
...
Now do a **deeper** codebase exploration than what `/skills aif-plan` did initially:
...
```
User: /skills aif-improve добавь обработку ошибок и валидацию входных данных
...
- /skills aif-plan full <description>
- /skills aif-plan fast <description>
- /skills aif-fix <bug description>"
```

### Example 4: Explicit plan file

```
User: /skills aif-improve @my-custom-plan.md add rollback and edge-case handling
...
Use:
- /skills aif-improve @.ai-factory/plans/feature-user-auth.md
- /skills aif-improve add validation and retries
```

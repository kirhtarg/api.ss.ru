...
- This reduces ambiguity in `/skills aif-implement` milestone completion and `/skills aif-verify` roadmap gates

**OPTIONAL (recommended):** Read the resolved research path if it exists:

- Treat `## Active Summary (input for /skills aif-plan)` as an additional requirements source
...
4. Roadmap milestone linkage (only if the resolved roadmap artifact exists):
   ...
   b. Skip — no linkage (allowed; `/skills aif-verify --strict` should report WARN, not fail, for missing linkage alone)
...
Store all preferences — they will be used in the plan file and passed to `/skills aif-implement`.

Docs policy semantics:

- `Docs: yes` → `/skills aif-implement` MUST show a mandatory documentation checkpoint and route docs changes through `/skills aif-docs`
- `Docs: no` (or unset) → `/skills aif-implement` emits `WARN [docs]` and continues without a mandatory docs checkpoint
...
To manage worktrees later:
  /skills aif-plan --list
  /skills aif-plan --cleanup <branch-name>
```
...
**Full mode + parallel (`--parallel`):** Automatically invoke `/skills aif-implement` — the whole point of parallel is autonomous end-to-end execution in an isolated worktree.

```
/skills aif-implement

CONTEXT FROM /skills aif-plan:
...
9. **Ownership boundary** – This command owns plan files only (the resolved fast plan path and files under `paths.plans`). Use owner commands (`/skills aif-roadmap`, `/skills aif-rules`, `/skills aif-explore`) for their artifacts.
...
- `/skills aif-implement` may offer deletion after completion

...
description: >-
  ...
  Use after "/skills aif-implement" completes, or when user says "verify", "check work", "did we miss anything".
...
**This skill is optional** — invoked after `/skills aif-implement` finishes all tasks, or manually at any time.
...
Same logic as `/skills aif-implement`:
...
2. If the branch-based plan is missing or git mode is off:
   ...
   → If multiple exist, ask the user to choose or use `@<path>` via `/skills aif-implement`
...
`/skills aif-verify` is **read-only** for context artifacts. Do not edit or regenerate `.ai-factory/*` files here.
...
- `DESCRIPTION.md` drift → suggest `/skills aif` (or note that `/skills aif-implement` should have updated it during implementation)
- `ARCHITECTURE.md` drift → suggest `/skills aif-architecture`
- `ROADMAP.md` drift → suggest `/skills aif-roadmap check` (or `/skills aif-roadmap <update request>`)
- `RULES.md` drift → suggest `/skills aif-rules <rule text>`
...
Options:
1. Fix now (recommended) — Use /skills aif-fix to address all issues
2. Fix critical only — Use /skills aif-fix for incomplete tasks, skip warnings
3. Fix directly here — Address issues in this session without /skills aif-fix
4. Accept as-is — Mark everything as done, move on
```

**If "Fix now" or "Fix critical only":**
- First suggest using `/skills aif-fix` and pass a concise issue summary as argument
- Example:
  - `/skills aif-fix complete Task #3 password reset email flow, implement Task #8 docs update, remove unfinished markers in src/services/auth.ts and src/middleware/rate-limit.ts, document SENDGRID_API_KEY in .env.example`
- If user agrees, proceed via `/skills aif-fix`
- If user declines `/skills aif-fix`, continue with direct implementation in this session
- For each incomplete/partial task — implement the missing pieces (follow the same implementation rules as `/skills aif-implement`)
...
- If unresolved issues remain (accepted or deferred), suggest `/skills aif-fix` first
...
1. 🛠️ /skills aif-fix [issue summary] — Fix remaining verification issues
2. 🔒 /skills aif-security-checklist — Run security audit on the new code
3. 👀 /skills aif-review — Code review of the implementation
4. 💾 /skills aif-commit — Commit the changes
...
Options:
1. Fix issues — Run /skills aif-fix with verification findings
2. Security check — Run /skills aif-security-checklist on changed files
3. Code review — Run /skills aif-review on the implementation
4. Both — Run security check, then code review
5. Skip — Proceed to commit
```

**If fix issues selected** → suggest invoking `/skills aif-fix <issue summary>`
**If security check selected** → suggest invoking `/skills aif-security-checklist`
**If code review selected** → suggest invoking `/skills aif-review`
**If both** → suggest security first, then review
**If skip** → suggest `/skills aif-commit`

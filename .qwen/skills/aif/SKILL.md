...
3. Generating custom skills via `/skills aif-skill-generator`
...
- If not found — ask the user via `AskUserQuestion`:
  ...
  3. Install Python first and re-run `/skills aif`
...
- "Install Python first" → **STOP**, user will re-run `/skills aif` after installing
...
- Never run blocking security decisions on built-in AI Factory skills (`~/.qwen/skills/skills aif` and `~/.qwen/skills/skills aif-*`).
...
```bash
$PYTHON ~/.qwen/skills/skills aif-skill-generator/scripts/security-scan.py <installed-skill-path>
```
...
**Both levels must pass.** See [skill-generator CRITICAL section](../skills aif-skill-generator/SKILL.md) for full threat categories.
...
This file contains project-specific rules accumulated by `/skills aif-evolve` from patches,
...
For each recommended skill:
  ...
  4. If not found → Generate: /skills aif-skill-generator <name>
  5. Has reference URLs? → Learn: /skills aif-skill-generator <url1> [url2]...
```
...
Immediately after determining Mode 1, Mode 2, or Mode 3, resolve the project language settings for the entire `/skills aif` run.
...
- `language.artifacts` — use for all setup-time text artifacts created in this run: `.ai-factory/DESCRIPTION.md`, `.ai-factory/rules/base.md`, `AGENTS.md`, and `.ai-factory/ARCHITECTURE.md` via `/skills aif-architecture`
...
6. Keep the resolved language state fixed for the entire `/skills aif` run. Do not generate setup-time text artifacts in a different language later in the same run.
...
```
AskUserQuestion: What UI language should I use for communication during this `/skills aif` run?
...
```
AskUserQuestion: What artifact language should I use for generated files in this `/skills aif` run?
...
2. If git is enabled, detect the default/base branch from git metadata:
   ...
   - Fallback to `main`
3. If git is enabled, ask whether `/skills aif-plan full` should create a new branch:

```
AskUserQuestion: How should full plans behave in git?

Options:
1. Create a new branch (Recommended) - /skills aif-plan full creates a branch and saves the full plan as a branch-scoped file
2. Stay on the current branch - /skills aif-plan full still creates a rich full plan, but without creating a new branch
```
...
- This write MUST happen before writing the first setup artifact and before invoking `/skills aif-architecture`.
...
```bash
node ~/.qwen/skills/skills aif/references/update-config.mjs \
  --template ~/.qwen/skills/skills aif/references/config-template.yaml \
...
- Managed keys for this helper are limited to:
  ...
  - `rules.base`
- Never normalize or overwrite `rules.<area>` entries. Those belong to `/skills aif-rules`.
...
**Trigger:** `/skills aif` (no arguments) + project has config files
...
3. Run `node ~/.qwen/skills/skills aif/references/update-config.mjs --template ~/.qwen/skills/skills aif/references/config-template.yaml --target .ai-factory/config.yaml --payload .ai-factory/config.update.json`
...
7. For each external skill from skills.sh:
   ...
   $PYTHON ~/.qwen/skills/skills aif-skill-generator/scripts/security-scan.py <installed-path>
   ```
...
8. Generate custom skills via `/skills aif-skill-generator` (pass URLs for Learn Mode when docs are available)
9. Configure MCP in `.qwen/settings.json`
10. Generate `AGENTS.md` in project root in resolved `language.artifacts` (see [AGENTS.md Generation](#agentsmd-generation))
11. Generate architecture document via `/skills aif-architecture` only after config exists with resolved language settings (see [Architecture Generation](#architecture-generation))

---

### Mode 2: New Project with Description

**Trigger:** `/skills aif <project description>`
...
Install skills, configure MCP, generate `AGENTS.md` in resolved `language.artifacts`, and generate architecture document via `/skills aif-architecture` after the earlier helper-driven config write, as in Mode 1.

---

### Mode 3: Interactive New Project (Empty Directory)

**Trigger:** `/skills aif` (no arguments) + empty project (no package.json, composer.json, etc.)
...
- The Documentation section will be maintained by `/skills aif-docs`
...
4. **MCP in `.qwen/settings.json`** — Project-level MCP configuration
...
- Delegated ownership: invoke `/skills aif-architecture` to create/update `.ai-factory/ARCHITECTURE.md`.
...
Invoke `/skills aif-architecture` to define project architecture. This creates `.ai-factory/ARCHITECTURE.md` with architecture pattern, folder structure, dependency rules, and code examples tailored to the project.
...
- [Localized next-steps heading in `language.ui`]:
  - `/skills aif-roadmap` — [Localized roadmap recommendation in `language.ui`]
  - `/skills aif-plan <description>` — [Localized planning recommendation in `language.ui`]
  - `/skills aif-implement` — [Localized execution recommendation in `language.ui`]
```

**For existing projects (Mode 1), also suggest next steps:**

Present these suggestions in resolved `language.ui`:
- `/skills aif-docs` — [Localized documentation recommendation in `language.ui`]
- `/skills aif-rules` — [Localized rules recommendation in `language.ui`]
- `/skills aif-build-automation` — [Localized build-automation recommendation in `language.ui`]
- `/skills aif-ci` — [Localized CI recommendation in `language.ui`]
- `/skills aif-dockerize` — [Localized containerization recommendation in `language.ui`]

Present these as `AskUserQuestion` with multi-select options:
1. [Localized docs option label in `language.ui`] (`/skills aif-docs`)
2. [Localized build-automation option label in `language.ui`] (`/skills aif-build-automation`)
3. [Localized CI option label in `language.ui`] (`/skills aif-ci`)
4. [Localized docker option label in `language.ui`] (`/skills aif-dockerize`)

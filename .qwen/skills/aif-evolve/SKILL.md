...
- `/skills aif-evolve` is the primary raw-patch analyzer. It processes patches **incrementally** using a cursor.
- `/skills aif-implement`, `/skills aif-fix`, and `/skills aif-improve` should prefer skill-context first; raw patches are fallback context only.
...
(check `.qwen/skills/<resolved-name>/SKILL.md` or `skills/<resolved-name>/SKILL.md`).
If the skill is not found → report an error to the user and stop:
"Skill '<resolved-name>' not found. Use `/skills aif-evolve` without arguments to evolve
...
- If evolving a **specific skill** (e.g., `/skills aif-evolve plan`) → read only:
...
- If evolving **all skills** (`/skills aif-evolve` or `/skills aif-evolve all`) → read all context files:
...
7. Full rescan procedure: delete `<resolved evolutions dir>/patch-cursor.json`, then run `/skills aif-evolve` again.
...
(e.g., for `/skills aif-plan` — focus on file structure and naming; for `/skills aif-fix` — error handling and testing).

### Step 2: Read Target Skills

**Read ONLY the base SKILL.md files for target skills — not all skills.**

- If evolving a **specific skill** (e.g., `/skills aif-evolve plan`) → read only that one:
  `Read: .qwen/skills/skills aif-plan/SKILL.md` (or `skills/aif-plan/SKILL.md` if not installed)
- If evolving **all skills** (`/skills aif-evolve` or `/skills aif-evolve all`) → read all:
  `Glob: .qwen/skills/*/SKILL.md` (or `Glob: skills/*/SKILL.md` if not installed)
...
##### /skills aif-plan — Fully covered: [Rule Name]
...
##### /skills aif-plan — Conflict: [Rule Name]
- **Base SKILL.md says:** [base rule text]
- **Skill-context says:** [project rule text]
...
##### /skills aif-fix — Partial overlap: [Rule Name]
- **Base SKILL.md says:** [base rule text]
- **Skill-context says:** [project rule text]
...
> Auto-generated by `/skills aif-evolve`. Do not edit manually.
...
1. **Run `/skills aif-review`** on recent code to verify improvements
2. **Next evolution** — run `/skills aif-evolve` again after 5-10 more fixes
3. **Consider new skill** — if pattern X keeps recurring, create a dedicated skill:
   `/skills aif-skill-generator <skill-name>`
```
...
10. **No new skills** — suggest `/skills aif-skill-generator` instead
...
```
/skills aif-evolve fix

→ Found 6/10 patches tagged #null-check
→ Improvement for /skills aif-fix (2 rules):
  Target: .ai-factory/skill-context/aif-fix/SKILL.md

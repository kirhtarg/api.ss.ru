...
- Each milestone is a **high-level goal**, not a granular task (that's `/skills aif-plan`)
...
/skills aif-plan <milestone description>  → creates a plan and optional branch/worktree flow
/skills aif-implement                     → executes the plan
```

---

### Mode 3: Check Progress (`/skills aif-roadmap check`)

Automated scan — analyze the codebase and mark completed milestones without interactive questions.

**Requires** the resolved roadmap path to exist. If it doesn't — tell the user to run `/skills aif-roadmap` first.
...
5. **NO implementation** — this skill only plans, use `/skills aif-plan` to start a feature and `/skills aif-implement` to execute
6. **Ownership boundary** — this command owns roadmap structure/content; `/skills aif-implement` may only mark milestones completed when implementation evidence is clear
